            ...
            for client in clients:
                if client != client_socket:
                    try:
                        ...
                        # (Dòng 30 từ hình ảnh thứ hai)
                        clients.remove(client)
    except: # Dòng này được suy ra từ hình ảnh thứ hai, nơi có 'except:' tiếp theo
        # (Dòng 32 từ hình ảnh thứ hai)
        clients.remove(client_socket)
    finally:
        # (Dòng 35 từ hình ảnh thứ hai)
        print("Đã ngắt kết nối:", client_socket.getpeername())
        # (Dòng 36 từ hình ảnh thứ hai)
        clients.remove(client_socket)
        # (Dòng 37 từ hình ảnh thứ hai)
        client_socket.close()
